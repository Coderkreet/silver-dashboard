...
const CompleteWithdrawalRequest = () => {
  ...
  const fetchWithdrawalHistory = async () => {
    try {
      ...
      // Filter only approved withdrawals
      const approvedWithdrawals = response?.withdrawals?.filter(
        (withdrawal) => withdrawal.status === 'approved'
      ) || [];
      setData(approvedWithdrawals);
    } catch (error) {
      console.error(error);
    } finally {
      setLoading(false);
      ...
      <div className="WithdrawalReport martop">
        <div className="dataTable ss-card martop">
          <DataTable
            ...
            emptyMessage="No approved withdrawals found."
